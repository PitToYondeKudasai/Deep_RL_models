...
class Maze():
    ...
    def reset(self, reset_maze: bool=False, n_edges_remove: int=3, reset_agent_goal_pos: bool=False):
        self.done = False
        if reset_maze:
    ...
    def plot_maze(self, height=5, width=5, states=True, with_agent=False, with_goal=False, save_path=None):
        if states:
        ...
        if with_agent:
            agent_circle = plt.Circle(self.states[self.agent_position], 5, color='r', fill=False)
