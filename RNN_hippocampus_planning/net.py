...
class ACModel(nn.Module):
    ...
    def forward(self, *args, best_action=False, simulation=False):
        ...
        pi = self.fc_pi(out)
        if simulation:
            pi = F.softmax(pi[:, :, :-1], dim=2).squeeze()
        else:
...
class ACModel_no_plan(nn.Module):
    def __init__(self, input_dim, env_size, action_dim, mem_out_size=128):
        super(ACModel_no_plan, self).__init__() 
        self.env_size = env_size
        self.action_dim = action_dim
    ...
    def forward(self, *args, best_action=False):
        x = args[0] 
        h_in, c_in = args[-2:]
        ...
        if not best_action:
            ...
            action = pi.argmax().item()
        return pi, v, action, h_out, c_out